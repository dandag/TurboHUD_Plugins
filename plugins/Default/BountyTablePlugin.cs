...
namespace Turbo.Plugins.Default
{
    public class BountyTablePlugin : BasePlugin, ITransparentCollection, ITransparent, IKeyEventHandler, INewAreaHandler, IInGameTopPainter
    {
		...
		private string evidenziaExpChest( string name ) {
			string ris = name.Replace("Bounty: ", "").Trim();
			
			switch( ris ) {
				case "The Cursed Cellar":
				case "The Cursed Court":
				case "The Cursed Bellows":
				case "The Cursed Pit":
				case "The Cursed Spire":
				case "The Cursed Moors":
				case "The Cursed Temple":
				case "The Cursed Garrison":
				case "The Cursed Bailey":
				case "The Cursed Peat":
					ris = "** " + ris + " **";
					break;				
			}
            ...
            for (var act = 1; act <= 5; act++)
            {
                ...
                foreach (var bountyType in BountyTypes)
                {
                    foreach (var quest in Hud.Game.Quests.Where(quest => quest.SnoQuest.Type == QuestType.Bounty))
                    {
						...
						string bountyName = evidenziaExpChest( quest.SnoQuest.NameLocalized );
						var textLayout = tf.GetTextLayout(bountyName);
